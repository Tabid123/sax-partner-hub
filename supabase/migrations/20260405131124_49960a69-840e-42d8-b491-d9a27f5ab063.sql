...
BEGIN
  -- Correct timezone: midnight Mogadishu → UTC
  v_today_start := date_trunc('day', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';

  IF p_period = 'today' THEN
    v_start := v_today_start;
    v_end := NULL;
  ELSIF p_period = 'yesterday' THEN
    v_start := v_today_start - interval '1 day';
    v_end := v_today_start;
  ELSIF p_period = 'week' THEN
    v_start := v_today_start - interval '6 days';
    v_end := NULL;
  ELSIF p_period = 'month' THEN
    v_start := date_trunc('month', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
    v_end := NULL;
  ELSIF p_period = 'year' THEN
    v_start := date_trunc('year', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
    v_end := NULL;
  ELSE
    v_start := NULL;
    v_end := NULL;
